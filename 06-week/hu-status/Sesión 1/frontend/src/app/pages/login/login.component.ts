...
export class LoginComponent {
  ...
  // Entrada real desde Home ("Acceso para empresas" -> /login?as=staff): solo
  // preselecciona la pestana visual de ayuda; el rol real lo sigue devolviendo el
  // Backend en el login, exactamente igual que si el usuario hubiera hecho clic en
  // la pestana "Equipo del operador" manualmente.
  role = signal<LoginRole>(this.route.snapshot.queryParamMap.get('as') === 'staff' ? 'staff' : 'client');
  ...
  // Preserva la intencion "equipo del operador" al ir a /recuperar y volver: mismo
  // mecanismo de query param ?as=staff que ya usa Home -> Login, sin nuevo contrato.
  recoverQueryParams = computed(() => (this.role() === 'staff' ? { as: 'staff' } : {}));
  ...
  onSubmit(event: Event): void {
    ...
    if (!email || !password) {
      this.setFeedback('Ingresa tu correo y tu contraseña.', true);
      return;
    }
