...
export class ClientTourBookingComponent implements OnInit {
  ...
  // Solo presentacion: etiquetas derivadas para el resumen lateral, sin alterar
  // validate()/onSubmit() ni el valor real de los signals que ya existian.
  departureLabel = computed(() => (this.departure() ? formatOperatorDate(this.departure()) : ''));
  depositLabel = computed(() => formatCurrency(this.depositAmount()));
  selectedTransportLabel = computed(() => {
    const id = this.selectedTransportItemId();
    if (!id) return null;
    return this.transportOptions().find((option) => option.catalogItemId === id)?.name ?? null;
  });
