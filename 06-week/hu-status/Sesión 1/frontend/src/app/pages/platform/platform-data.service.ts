...
export class PlatformDataService {
  ...
  // Altas reales del mes calendario actual (America/Bogota), a partir de createdAt real
  // de TenantController (GET /api/tenants). Reemplaza el conteo total historico de tenants
  // que antes se mostraba bajo la etiqueta "Altas este mes" (medía otra cosa).
  readonly newTenantsThisMonth = computed(() => {
    const now = new Date();
    const currentYear = Number(new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Bogota', year: 'numeric' }).format(now));
    const currentMonth = Number(new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Bogota', month: 'numeric' }).format(now));
    return this.tenantsRawSignal().filter((tenant) => {
      const created = new Date(tenant.createdAt);
      if (Number.isNaN(created.getTime())) return false;
      const createdYear = Number(new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Bogota', year: 'numeric' }).format(created));
      const createdMonth = Number(new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Bogota', month: 'numeric' }).format(created));
